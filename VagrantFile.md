...
## Ubuntu  
**WordPress**  
1. **vagrant init "any_ubuntu_20"** - Create a vagrantfile
2. Open the file and uncomment public & private ip. Set the static to 192.168.56.any
3. Uncomment provisioning part and add all the manual steps as code
```
...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "web" do |web|
    ...
    web.vm.provision "shell", inline: <<-SHELL
      # Commands to provision the "web" VM
      sudo apt update
      sudo apt install -y \
        apache2 \
        php \
  ...
  config.vm.define "db" do |db|
    ...
    db.vm.provision "shell", inline: <<-SHELL
      # Commands to provision the "db" VM
      sudo apt update
...
**vagrant ssh "VM_Name"** - Choose which one to connect too
